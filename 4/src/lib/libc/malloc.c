...
__malloc_init(void)
{
	...
	x = sbrk(0);
	if (x==(void *)-1) {
		err(1, "A3malloc: initial sbrk failed");
	}
	if (x==(void *) 0) {
		errx(1, "malloc: Internal error - heap began at 0");
	}
        printf("printing the after\n");
	...
	if (__heapbase % MBLOCKSIZE != 0) {
		size_t adjust = MBLOCKSIZE - (__heapbase % MBLOCKSIZE);
	        printf("printing the value of adjust:%d\n",adjust);
          	x = sbrk(adjust);
                printf("printing to check pass offfffffffff second sbrk\n");
		if (x==(void *)-1) {
...
{
	void *x;
   	printf("Entering sbrk through malloc_sbrk\n"); 
	x = sbrk(size);
	if (x == (void *)-1) {
		return NULL;
	}
...
malloc(size_t size)
{
	...
	if (i!=__heaptop) {
    	...
    	printf("Out of malloc dump\n");
	/*
	 * Didn't find anything. Expand the heap.
	 */
    
	mh = __malloc_sbrk(size + MBLOCKSIZE);
	printf("malloc_sbrk value of mh %d",mh);
	if (mh == NULL) {
		return NULL;
	}
    	printf("Out of malloc_sbrk");
